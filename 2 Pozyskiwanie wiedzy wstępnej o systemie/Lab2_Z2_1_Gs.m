...
G_trans = tf(0.5,[5 11 7 1],'OutputDelay',3);
...
h_hat = [h_hat sum(ghat_11(1:i))]; 
end

Wykres3=figure('Position',[0 0 1600 1000]);

plot(0:length(h_hat)-1,h_hat,'LineWidth',3)
hold on
plot(0:length(h_hat)-1,h_org(1:length(h_hat)),'LineWidth',3)
set(gca,"FontSize",12)
xlabel("probka [n]","FontSize",19,"Interpreter","latex")
ylabel("wartosc estymaty","FontSize",19,"Interpreter","latex")
title("Estymata odpowiedzi skokowej systemu","FontSize",22,"Interpreter","latex")
set(gca,"TickLabelInterpreter","latex")
lgd = legend('$\hat{h}(n)$','$\hat{h_{2}}(n)$','Interpreter','Latex');
fontsize(lgd,14,"points")
grid on
hold off