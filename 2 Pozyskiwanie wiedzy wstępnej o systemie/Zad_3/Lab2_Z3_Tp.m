...
Tp_vec = [2 1 0.5 0.1];
xlim_vec = [500 400 400 200];
for o = 1:length(Tp_vec)
Mw = 200;   
Tp = Tp_vec(o);
new_xlim = xlim_vec(o);
TF = 2.5*Tp/(2*pi);
...
subplot(1,2,1)
sgtitle("Estymowane wykresy bodego dla $T_p$ = " + num2str(Tp),'FontSize',16,'interpreter','latex')
plot(omega_k1(1:300),LM1(1:300),'LineWidth',3);
hold on
plot(omega_k2,LM2,'LineWidth',3);
plot(wout,mag,'LineWidth',3)
ylim([-50 10]);
xlim([wout(1) wout(new_xlim)])
